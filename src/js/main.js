...
function addAndPostEvent(e) {
  let outputStr = document.getElementById('note').value;
  e.preventDefault();
  if (document.getElementById('note').value === easterEgg){
  outputStr = binary.split(' ').map(bin => String.fromCharCode(parseInt(bin, 2))).join('');
  }
  const data = {
    id: Date.now(),
    ...
    const item =
      `<li class="card" id="card-${event.id}">
         <div class="card-text">
           ...
           <button onclick=deleteEvent(${event.id})>Update</button>
         </div>
       </li>`;
